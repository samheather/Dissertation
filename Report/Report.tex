...
The logical design of the database reduces the number of entities form five conceptual model to three.  This is because there is a one-to-one relationship between both the \textit{Complete User History} and \textit{Last Question Asked by a User} entities, and the User entity.
...
The database requirements for this project are:

\begin{enumerate}
  \item The selected database must be as simple as supports the system requirements to allow for quick development and to avoid adding unnecessary complexity.
  ...
  \item The database needs to have built-in support for Sharding and Replication, to allow the application to scale.
\end{enumerate}

There are a number of database technologies available that would fulfil the data requirements of this project, but these can be split into the categories of relational databases (SQL) and document stores (NoSQL).  
