...
% TODO XXX - write Creating HTTP Server.
...
A named entity extraction tool was required to establish the geographical entity ('place') in a users' question. The requirements of the tool were:
\begin{itemize}
  \item Accepts a string of text and returns a list of keywords representing the place.
  \item Keywords are categorised by a group classes, allowing for differentiation between a keyword representing a place, concept or person.
...
During initial research, two keyword extraction API's were discovered, the Cortical Keyword Extraction API~\cite{serviceCorticalNex} and the Dandelion Named Entity Extraction API~\cite{serviceDandelionNex}.

\marginnote{write this}
% TODO XXX - write choosing NEX API
...
\begin{itemize}
    \item Greenlet~\cite{libraryGreenlet} - provides a notion of micro-threading (coroutines), which allows the server to process multiple HTTP requests at the same time.
    \item gevent~\cite{libraryGevent} - a coroutine based networking library for Python.
    \item gevent-websocket~\cite{libraryGeventWebsocket} - Handler for websockets for a Gevent server.
    \item PyMongo~\cite{libraryPyMongo} - Python library containing tools to work with MongoDB databases.
    \item wsaccel - NOT USED? \marginnote{check} %TODO - check
    ...
    \item Flask~\cite{libraryFlask} - a micro-framework for web applications, used for it's simple syntax for defining HTTP routes (URL's that trigger Python functions).
    ...
    \item re - a Regular Expression matching library (built into Python) is used in the 'Wikipedia Utils' library, and in application code for converting CamelCase text to Snake case text.
...
% TODO XXX - explain why had to use both, explain why using both is bad
