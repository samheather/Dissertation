...
\titleformat{\part}%[frame]
  {\normalfont}
  {\filcenter\placeabove[2\baselineskip]{\Large}{\huge\enspace\enspace}}
  {0pt}
  {\headerdisplay}
...
% No research has been undertaken investigating whether bringing knowledge through this method is a viable option.
...
\section{Semantically Comparing Text}
\label{sec:litRevSemantics}
...
A significant issue that is present in statistical machine learning systems comes from the knowledge we assume an individual has and derives from a word~\cite{kenny2011ethics}; in this project, this affects the translation of the answers returned to a user.  In human translation this is solved by the translator's knowledge of the difference in material culture, allowing them to append necessary information to the resulting translation that the recipient might find useful~\cite{kenny2011ethics}.  In statistical machine learning, cultural awareness of material knowledge is a separate problem on its own~\cite{kenny2011ethics}.  To take one example from Melby (2006):
...
\caption{Component View of the system in ArchiMate notation~\cite{archiMateSpec} showing the modular structure of the application. The diagram shows the modules used when computing an answer to a question and the interfaces between them.}
...
\begin{enumerate}
  ...
  \item Common libraries that will be used for creating a HTTP server, making database queries and sending SMSs tend to be written primarily on the Linux platform.
...
\caption{Deployment View of the system in ArchiMate notation~\cite{archiMateSpec}. The diagram shows the structure of the main server, which both runs the application and hosts the database.}
...
A library called Gevent will be used to replace Werkzeug in Flask as it does support concurrent HTTP connections~\cite{geventImplementingServers} using using the Greenlet library~\cite{libraryGreenlet}, which provides a notion of micro-threading (coroutines). When a new HTTP request is received, Gevent creates a new Greenlet~\cite{geventImplementingServers}, thus preventing the main thread being blocked and allowing multiple concurrent HTTP requests. 
...
Two services were considered that matched these criteria.  These were Twilio~\cite{serviceTwilio} and Plivo~\cite{servicePlivo}. Both use REST APIs for handling incoming messages and for sending responses. Both Twilio and Plivo have comparable coverage as well: Twilio state that they send and receive SMS with numbers in 198 countries~\cite{twilioCoverage}, and Plivo state that they support 207 countries~\cite{plivoCoverage}. The difference is trivial, as the number of recognised independent states in the world is between 190 and 200 depending on listing; the US Government recognises 195~\cite{usStateDepartmentListOfIndependentStates}.
...
\begin{figure}[htb] 
\centerline{\includegraphics[width=0.96\linewidth]{systemSmsInterface}}
\caption{Application Behaviour View of the system in ArchiMate notation~\cite{archiMateSpec}. This diagram shows the question/answer service provided by the application and the SMS interface to it. The HTTP interface used for development and testing is also shown for completeness.~\cite{archiMateSpec}}
...
\section{Implementation of rating and machine learning system}
...
The machine learning system built uses Machine Learning from the Reinforcement Learning paradigm, as described in the Literature Review in section \ref{sec:litRevML}, as this suits the concept of users providing feedback which only indicates if an answer was of high or low quality. The general technique used is that of Learning by Reward/Punishment, also described in section \ref{sec:litRevML}, where ratings in the database are used as weightings to adjust the parameters in the answer location process.
...
In this equation, \texttt{n} is the number of participants, \texttt{x} is the participant ID, and \texttt{y} is the mean usefulness rating calculated for an individual participant. The PCC gives a value in the range of \texttt{-1}, which indicates a perfect negative correlation, to \texttt{+1}, which indicates a perfect positive correlation, with zero indicating no correlation at-all. Using this equation the correlation coefficient for this dataset was found to be 0.506.

A graph showing usefulness plotted against the participant number is shown in figure \ref{fig:usefulnessTrend} Using linear regression a line of best fit was added to the graph with the equation $y = 0.075 x + 2.68$.
...
14  & 0.240 \\ \hline
\end{tabular}}
\caption{Table showing the Pearson product-moment correlation coefficient of the answers given for the individual usefulness questions against the number of preceding participants.}
\label{table:pccOfUsefulnessQuestions}
\end{table}
...
\blockquote{I thought the detail generated from the answers to the describe questions were really detailed and informative, although displaying the units in other answers would have been helpful.} - Participant fourteen.

\blockquote{Answers with units would be helpful to understand values given as answers} - Participant nine.
...
The participant pool contained 21 individuals and was skewed towards students in the age range of 18-24 who used the internet multiple times per day. This is not representative of the range of possible users of the application developed, notably the personas developed in appendix \ref{sec:appendixPersonas} which were used to drive the user requirements. As the participant pool was skewed towards a young student population with regular access to the internet it is suggested that some had difficulty imagining themselves working in the context of not having access to the Internet.
