...
Initially this project was about providing medical information however the ethical issues were too significant to handle within the parameters of this project. Ethical issues that arose were: information provided in a medical context could lead to a user taking a harmful or life-changing action if the information was incorrect, meaning that the application developed had the potential to cause physical harm; and communicating questions and answers between a user and the service in a secure manner is not possible over SMS, which does not support encryption, meaning user privacy does not meet appropriate standards.
...
These values were found using trial and adjustment but were tested with two particular use cases: 

When \texttt{population} is requested, the value for \texttt{areaUrban} is returned with semantic similarity of 0.30488 (5 significant figures) instead of \texttt{population\_urban}, which also has a semantic similarity of 0.30488.

When the property \texttt{tall} is requested of the Leaning Tower of Pisa (in the context of \texttt{how tall is...}), the value for \texttt{architect} is returned with semantic similarity of 0.25305 XXXXXX instead of \texttt{population\_urban}, which also has a semantic similarity of 0.30488.
...
In this section the results from the experiment are presented and analysed. The key piece of research been investigated is whether the application learns effectively, although results for general usefulness and usability are also presented.

\subsection{Usefulness}
\label{subsec:resultsUsefulness}

The usefulness of the system is measured by questions 1 through 6, 13 and 14 in the questionnaire. In section \ref{sec:evaluatingSuccess} the success criteria for usefulness is defined as a significant positive average rating for the usefulness questions. The average of usefulness ratings was 3.51 (3.s.f.) with a standard deviation of 0.919. This shows a slightly-above neutral rating of the system. One standard deviation of the data from the mean (containing 68\% of the data points) put the usability of the system in the range 2.59 through 4.43, which shows that the data was moderately spaced out in the range - (it was not highly concentrated or well distributed in the range of possible ratings).

\subsection{Usability}
\label{subsec:resultsUsability}

The usability of the system is measured by questions 7 through 12 in the questionnaire. In section \ref{sec:evaluatingSuccess} the success criteria for usability is defined as a significant positive average rating for the usability questions. The average of usability ratings was 3.46 (3.s.f.) with a standard deviation of 0.810. This shows a slightly-above neutral rating of the system. One standard deviation of the data from the mean (containing 68\% of the data points) put the usability of the system in the range 2.65 through 4.27, which shows that the data was moderately spaced out in the range - (it was not highly concentrated or well distributed in the range of possible ratings).

\subsection{Machine Learning}
\label{subsec:resultsMachineLearning}

The criteria that would be measured in this experimental evaluation was set out in section \ref{sec:evaluatingSuccess}. The data was inspected from two view points: the usefulness of the application and the usability of the application.

As discussed in section \ref{sec:evaluatingSuccess}, the usefulness will be evaluated according to a positive trend in feedback from participants as the number of participants who have used the application prior to them increases. A mean value of usefulness was calculated using the values from questions 1 through 6, 13 and 14 and plotted. Linear regression was then carried out to identify whether a positive correlation existed showing that participants were indicating they would find the application more useful as more participants used it before them. The Correlation Coefficient for this dataset was 0.506, indicating a moderate positive trend. This implies that the application was learning and its usefulness improving as more participants used it.
...
\subsection{Additional comments from particiapnts}
...
Although these can't be included in any analysis due to not asking for a comment from all participants, both do raise the issue of units of measurement not been returned with an answer for some questions.

\section{Data loss}
\label{sec:dataLoss}

Log files were kept during the experiment containing the questions asked, answers provided and the ratings from a user to allow for the possibility of further analysis, such as the identification of specific questions that caused users a problem or questions where the answer returned changed as more participants took part. However due to a bug log files for participants up until participant number 14 were not stored. The seven remaining log files are too small a dataset to analyse.
